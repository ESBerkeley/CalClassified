...
class FacebookUserConverter(object):
   ...
   #     response = self.open_facebook.set('407167362711476/feed', link='http://buynear.me'+item.get_absolute_url(), message='Selling '+ item.title+'!') #Testing 2050 group
        response = self.open_facebook.set('266259930135554/feed',
                                          ...
                                          message='Selling '+ item.title + ', $'+ str(item.price) + ' on buynear.me') #Real ffs group
        return response
    ...
    def set_fb_group(self, item, url):
        ...
        response = self.open_facebook.set(url,
                                          link='http://buynear.me'+item.get_absolute_url(),
                                          message='Selling '+ item.title + ', $'+ str(item.price) + ' on buynear.me')
        return response
    ...
    def _store_friends(self, user, friends):
        ...
        for friend in friends:
            if friend['id'] in allppl:
                ...
                userid = FacebookProfile.objects.filter(facebook_id = friend['id'])[0].user_id
                up = user.get_profile()
                FacebookUser(user_id = userid, facebook_id = up.facebook_id, name = up.facebook_name).save() #no gender for now, don't think we need that -rory
