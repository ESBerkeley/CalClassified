...
import json
from cc.django_facebook import settings as facebook_settings
from cc.django_facebook.utils import mass_get_or_create, cleanup_oauth_url, \
    get_profile_model, get_profile_class, parse_signed_request, hash_key, try_get_profile, \
    get_user_attribute
from cc.open_facebook.exceptions import OpenFacebookException
from cc.django_facebook.exceptions import FacebookException
try:
    from dateutil.parser import parse as parse_date
except ImportError:
    from cc.django_facebook.utils import parse_like_datetime as parse_date
from cc.django_facebook.utils import get_user_model

import datetime
import logging
from cc.open_facebook import exceptions as open_facebook_exceptions
from cc.open_facebook.utils import send_warning, validate_is_instance
from cc.django_facebook import signals
from cc.django_facebook import *
...
class FacebookUserConverter(object):
   ...
   #     response = self.open_facebook.set('407167362711476/feed', link='http://buynear.me'+item.get_absolute_url(), message='Selling '+ item.title+'!') #Testing 2050 group
    ...
    def get_and_store_groups(self, user):
        '''
        Gets and stores your facebook groups to DB
        Both the get and the store run in a async task when
        FACEBOOK_CELERY_STORE = True
        '''
        if facebook_settings.FACEBOOK_CELERY_STORE:
            from django_facebook.tasks import get_and_store_groups
            get_and_store_groups.delay(user, self)
        else:
            self._get_and_store_groups(user)

    def _get_and_store_groups(self, user):
        groups = self.get_groups()
        stored_groups = self._store_groups(user, groups)
        return stored_groups
        
    def get_groups(self, limit=100):
        '''
        Parses the facebook response and returns the groups
        '''
        groups_response = self.open_facebook.get('me/groups', limit=limit)
        groups = groups_response and groups_response.get('data')
        logger.info('found %s groups', len(groups))
        return groups
        
    def store_groups(self, user, groups):
        '''
        Given a user and groups store these in the db
        Note this can be a heavy operation, best to do it
        in the background using celery
        '''
        if facebook_settings.FACEBOOK_CELERY_STORE:
            from django_facebook.tasks import store_groups
            store_groups.delay(user, groups)
        else:
            self._store_groups(user, groups)
